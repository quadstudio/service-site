<tr>
    ...
    <td>
        <a href="{{ route('trades.show', $item) }}">{{ $item->name }}</a>
    ...
    <td class="text-right">
        <a class="btn btn-link" data-toggle="tooltip" data-placement="top" title="@lang('repair::messages.edit')"
           href="{{ route('trades.edit', $item) }}">
            <i class="fa fa-pencil" aria-hidden="true"></i>
        </a>
        @if($item->canDelete())
            <a class="btn btn-link text-danger btn-row-delete"
               data-form="#trade-delete-form-{{$item->id}}"
            ...
            <form id="trade-delete-form-{{$item->id}}"
                  action="{{route('trades.destroy', $item)}}"
                  method="POST">
                @csrf
                @method('DELETE')
            </form>

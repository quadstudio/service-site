@foreach($catalog->catalogs()->where('enabled', 1)->get() as $children)
    @if($children->catalogs()->count() == 0 && $children->equipments->count() > 0)
        <hr />
        <h3>{{ $children->parentTreeName() }}</h3>
        <div class="row">
            @foreach($children->equipments as $equipment)
                <div class="col-lg-3 col-md-4 col-sm-6 portfolio-item">
                    <div class="card h-100">
                        <a href="{{route('equipments.show', $equipment)}}"><img class="card-img-top" src="http://placehold.it/700x400" alt=""></a>
                        <div class="card-body">
                            <h4 class="card-title">
